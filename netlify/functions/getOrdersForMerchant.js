// Netlify Function: getOrdersForMerchant
// Fetches all orders being delivered to a specific merchant
// Query param: merchant_id
...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (!merchantId) {
      return { statusCode: 400, body: JSON.stringify({ error: 'merchant_id required' }) };
    }
    ...
    // Read from Orders sheet
    // Columns: A=order_id, B=agent_id, C=merchant_id, D=goods_type, E=weight, F=price, G=pickup_location, H=drop_location, I=status, J=created_at, K=driver_id
    ...
    const orders = rows
      .filter(row => (row[2] === merchantId || !row[2]) && ['accepted', 'picked'].includes(row[8])) // Filter by merchant_id (column C) and status in-transit
      .map(row => ({
        order_id: row[0] || '',
        agent_id: row[1] || '',
        merchant_id: row[2] || merchantId,
        goods_type: row[3] || '',
    ...
    console.error('getOrdersForMerchant error:', e);
