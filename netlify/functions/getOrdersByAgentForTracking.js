// Netlify Function: getOrdersByAgentForTracking
// Fetches all orders created by an agent (for tracking purposes, excluding pending only)
...
exports.handler = async function(event) {
  ...
  try {
    ...
    const orders = rows
      .filter(row => row[1] === agentId) // Filter by agent_id (column B)
      .map(row => ({
        order_id: row[0] || '',
        agent_id: row[1] || '',
        merchant_id: row[2] || '',
    ...
    console.error('getOrdersByAgentForTracking error:', e);
