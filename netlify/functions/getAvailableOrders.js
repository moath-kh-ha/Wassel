// Netlify Function: getAvailableOrders
// Fetches all orders with status 'pending' (available for drivers to accept)
// Returns all pending orders regardless of location (for MVP; can add location filtering later)
...
exports.handler = async function(event) {
  ...
  try {
    ...
    const orders = rows
      .filter(row => row[8] === 'pending') // Filter by status 'pending' (column I)
      .map(row => ({
        order_id: row[0] || '',
        agent_id: row[1] || '',
        merchant_id: row[2] || '',
